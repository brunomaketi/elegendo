...
const LABEL_VER_PLANOS = 'Ver planos'
const LABEL_UPGRADE = 'Fazer upgrade agora'
const LABEL_VER_CONTEUDO = 'Ver meu conteudo gerado'
const LABEL_GERAR = 'Gerar conteudo'
const LABEL_GERANDO = 'Gerando...'
const LABEL_COPIAR = 'Copiar conteudo'
const LABEL_GERAR_NOVO = 'Gerar novamente'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error ?? 'Erro ao gerar conteudo.')
        setLoading(false)
        return
      }
  ...
  return (
    <>
      <style>{`
        ...
        @media (min-width: 900px) { .agent-grid { grid-template-columns: 400px 1fr; } }
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
            ...
            <div>
              <p style={{ margin: 0, color: '#fff', fontSize: 13, fontWeight: 700, lineHeight: 1.3 }}>
                Ultima geracao gratuita usada!
              </p>
              <p style={{ margin: 0, color: 'rgba(255,255,255,0.75)', fontSize: 12, lineHeight: 1.3 }}>
                Faca upgrade e continue gerando conteudo ilimitado para sua campanha
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <a href="/planos?highlight=essencial" style={{
      ...
      {showUpgradeModal && (
        <div onClick={handleCloseModal} style={{
          ...
          padding: 20, animation: 'overlayIn 0.25s ease',
        }}>
          <div onClick={(e) => e.stopPropagation()} style={{
            ...
            <div style={{
              ...
              <h2 style={{ margin: '0 0 6px', color: '#fff', fontSize: 22, fontWeight: 800, fontFamily: 'var(--font-inter), sans-serif', lineHeight: 1.2 }}>
                Voce usou sua ultima geracao gratuita!
              </h2>
              <p style={{ margin: 0, color: 'rgba(255,255,255,0.75)', fontSize: 14, fontFamily: 'var(--font-inter), sans-serif', lineHeight: 1.5 }}>
                O conteudo ja foi gerado e esta disponivel abaixo.
              </p>
            ...
            <div style={{ padding: '24px 28px 28px' }}>
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 12, fontWeight: 600, color: 'rgba(45,27,110,0.5)', fontFamily: 'var(--font-inter), sans-serif' }}>
                  <span>Geracoes utilizadas</span>
                  <span style={{ color: '#7B4FD8' }}>5 de 5</span>
                </div>
                <div style={{ height: 8, background: 'rgba(123,79,216,0.1)', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: '100%', borderRadius: 4, background: 'linear-gradient(90deg, #7B4FD8, #C9A84C)' }} />
                </div>
              </div>

              <div style={{ background: 'rgba(123,79,216,0.05)', borderRadius: 14, padding: '16px 18px', marginBottom: 20, border: '1px solid rgba(123,79,216,0.12)' }}>
                <p style={{ margin: '0 0 10px', fontSize: 13, fontWeight: 700, color: '#2D1B6E', fontFamily: 'var(--font-inter), sans-serif' }}>
                  Com o plano Essencial voce tem:
                </p>
                {['50 geracoes por mes', 'Modelo de IA superior (Sonnet)', 'Conteudo de maior qualidade', 'Suporte prioritario'].map((b) => (
                  <div key={b} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, fontSize: 13, color: '#2D1B6E', fontFamily: 'var(--font-inter), sans-serif' }}>
              ...
              <a href="/planos?highlight=essencial" style={{
                ...
                marginBottom: 10, boxShadow: '0 6px 20px rgba(123,79,216,0.4)',
                fontFamily: 'var(--font-inter), sans-serif', animation: 'pulse 2s ease infinite',
              }}>
                {LABEL_UPGRADE}
              </a>
              <button onClick={handleCloseModal} style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {done && <span style={{ fontSize: 11, padding: '3px 10px', background: 'rgba(29,158,117,0.1)', color: '#1D9E75', borderRadius: 20, fontWeight: 700 }}>Concluido</span>}
          ...
          <div ref={outputRef} style={{ minHeight: 320, maxHeight: '65vh', overflowY: 'auto', background: output ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.5)', backdropFilter: 'blur(8px)', borderRadius: 18, padding: '20px', fontSize: 14, lineHeight: 1.9, color: '#2D1B6E', whiteSpace: 'pre-wrap', border: `1px solid ${done ? 'rgba(29,158,117,0.25)' : 'rgba(123,79,216,0.1)'}`, transition: 'border-color 0.3s' }}>
            {!loading && !output && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 260, gap: 12 }}>
                ...
                <p style={{ fontSize: 14, fontWeight: 600, color: '#2D1B6E', margin: 0 }}>O conteudo gerado aparecera aqui</p>
                <p style={{ fontSize: 13, color: 'rgba(45,27,110,0.4)', margin: 0, textAlign: 'center', maxWidth: 260, lineHeight: 1.6 }}>
                  Preencha os campos e clique em Gerar conteudo
                </p>
              </div>
            )}
            {loading && !output && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 260, gap: 16 }}>
                ...
                <p style={{ fontSize: 14, color: '#2D1B6E', margin: 0, fontWeight: 600 }}>Gerando seu conteudo...</p>
