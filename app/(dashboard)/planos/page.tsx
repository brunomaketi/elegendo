  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 28px', fontFamily: 'var(--font-inter), sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '48px', alignItems: 'start' }}>
        {PLANOS.map((plano) => (
          <div
            ...
            style={{
              background: plano.bg,
              border: `2px solid ${plano.border}`,
              borderRadius: '16px',
              padding: '28px 24px',
              position: 'relative',
              transform: plano.destaque ? 'scale(1.04)' : 'none',
              boxShadow: plano.destaque ? '0 8px 40px rgba(26,26,46,0.15)' : 'none',
            }}
          >
            {/* Badge */}
            {plano.badge && (
              <div style={{ position: 'absolute', top: '-13px', left: '50%', transform: 'translateX(-50%)', background: '#C9A84C', color: '#1A1A2E', fontSize: '11px', fontWeight: 700, padding: '4px 14px', borderRadius: '20px', whiteSpace: 'nowrap', letterSpacing: '0.04em' }}>
                {plano.badge}
              </div>
            )}
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '13px', fontWeight: 700, color: plano.destaque ? '#E8D5A3' : plano.cor, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '4px' }}>
                {plano.nome}
              </div>
              <div style={{ fontSize: '13px', color: plano.destaque ? 'rgba(232,213,163,0.6)' : '#8A8A9A' }}>
                {plano.descricao}
              </div>
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              {plano.preco === 0 ? (
                <div style={{ fontSize: '36px', fontWeight: 800, color: plano.destaque ? '#fff' : '#1A1A2E', lineHeight: 1 }}>
                  Grátis
                </div>
              ) : (
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                  <span style={{ fontSize: '14px', fontWeight: 600, color: plano.destaque ? '#E8D5A3' : '#8A8A9A', marginTop: '8px' }}>R$</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '24px' }}>
              {plano.beneficios.map((b, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', opacity: b.ok ? 1 : 0.35 }}>
                  ...
                  <span style={{ fontSize: '13px', color: plano.destaque ? (b.ok ? '#fff' : 'rgba(255,255,255,0.35)') : (b.ok ? '#1A1A2E' : '#8A8A9A') }}>
                    {b.texto}
                  </span>
                </div>
              ))}
            ...
            {/* CTA */}
            {planoAtual === plano.id ? (
              <div style={{ padding: '12px', borderRadius: '10px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', textAlign: 'center', fontSize: '13px', fontWeight: 600, color: plano.destaque ? 'rgba(255,255,255,0.5)' : '#8A8A9A' }}>
                Plano atual
              </div>
            ) : (
              <button
                onClick={() => alert('Stripe em breve!')}
                style={{
                  width: '100%', padding: '13px', borderRadius: '10px', border: 'none', cursor: 'pointer',
                  background: plano.destaque ? '#C9A84C' : plano.id === 'agencia' ? '#1D9E75' : '#1A1A2E',
                  color: plano.destaque ? '#1A1A2E' : '#fff',
      ...
      <div style={{ marginBottom: '40px' }}>
        <h2 style={{ fontSize: '20px', fontWeight: 700, color: '#1A1A2E', marginBottom: '16px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {FAQ.map((item, i) => (
            <div
              key={i}
              style={{ background: '#fff', border: '1px solid #E8E0D0', borderRadius: '10px', overflow: 'hidden' }}
            >
              <button
                onClick={() => setFaqAberto(faqAberto === i ? null : i)}
                style={{ width: '100%', padding: '16px 20px', background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontFamily: 'var(--font-inter), sans-serif' }}
              >
              ...
              {faqAberto === i && (
                <div style={{ padding: '0 20px 16px', fontSize: '14px', color: '#4A4A5A', lineHeight: '1.6' }}>
                  {item.r}
                </div>
              )}
            </div>
      ...
      <div style={{ textAlign: 'center', padding: '40px 24px', background: '#1A1A2E', borderRadius: '16px' }}>
        <div style={{ fontSize: '22px', fontWeight: 800, color: '#fff', marginBottom: '8px' }}>
          A janela fecha em outubro de 2026.
        </div>
        <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.6)', margin: '0 0 24px' }}>
          Cada semana sem estratégia digital é voto que o seu concorrente está conquistando.
        </p>
        <button
          onClick={() => alert('Stripe em breve!')}
          style={{ padding: '14px 32px', background: '#C9A84C', color: '#1A1A2E', borderRadius: '10px', border: 'none', fontSize: '15px', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-inter), sans-serif' }}
        >
          Começar agora →
        </button>
      </div>
